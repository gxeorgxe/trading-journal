...
export function SessionBarChart({ data }: Props) {
  ...
  return (
    <GlassCard>
      <h3 className="text-sm font-semibold text-gray-300 mb-1">R by Session</h3>
      <p className="text-xs text-gray-600 mb-4">Performance breakdown by market session</p>
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} margin={{ top: 4, right: 8, bottom: 4, left: 0 }}>
          <defs>
            <linearGradient id="sessionGreen" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#22c55e" stopOpacity={1} />
              <stop offset="100%" stopColor="#22c55e" stopOpacity={0.5} />
            </linearGradient>
            <linearGradient id="sessionRed" x1="0" y1="0" x2="0" y2="1">
          ...
          <Bar dataKey="totalR" radius={[4, 4, 0, 0]} maxBarSize={80} animationDuration={800}>
            {data.map((entry, i) => (
              <Cell key={i} fill={entry.totalR >= 0 ? 'url(#sessionGreen)' : 'url(#sessionRed)'} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
